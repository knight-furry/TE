...
public class Login extends JFrame {
	...
	public Login() {
		...
		username_title.setBounds(620, 290, 300, 35);
		username_title.setFont(new Font("Bradley Hand ITC", Font.BOLD, 23));
		contentPane.add(username_title);
		
		username = new JTextField();
		username.setBounds(620, 330, 300, 30);
		...
		reset_button.setBounds(620, 505, 75, 30);
		...
		login_button.setBounds(820, 505, 100, 30);
				...
				else
				{
					try{
						 ...
						 int flag = 0;
				            ...
				            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/student?autoReconnect=true&useSSL=false", "root", "datta123");
				            Statement st1 = con.createStatement();
				            ResultSet res = st1.executeQuery("select * from userdata");
				            ...
				            if(flag != 1)
			                {
			                    JOptionPane.showMessageDialog(null,"You got Bamboozled !");                  
			                }
				        }
				        catch(Exception ex)
				        {
		...
		register_title.setBounds(630, 580, 300, 35);
		register_title.setFont(new Font("Bradley Hand ITC", Font.BOLD, 25));
		contentPane.add(register_title);
		
		JButton register_button = new JButton("REGISTER");
		register_button.setBounds(720, 630, 100, 30);
